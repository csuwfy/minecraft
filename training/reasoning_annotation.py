...
def build_prompt(stage: int, task: str, actions: str) -> str:
    ...
    return (
        ...
        "objects, tools, inventory contents, equipment, or game state that cannot be inferred from the frames. "
        "Avoid naming specific tools such as axe, pickaxe, sword, hotbar, or inventory unless they are explicitly "
        "provided in the gold action JSON. Prefer neutral wording like move closer, adjust view, attack, use, or "
        "continue the task. Keep it one or two sentences.\n"
        f"{target}\n"
        "Prefer strict JSON with keys: reasoning, confidence, tags. If JSON formatting fails, output only the "
        "reasoning sentence without extra commentary.\n"
        f"Stage: {stage}\n"
